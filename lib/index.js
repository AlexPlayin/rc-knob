...
const reduceOnStart = (state, action, callbacks) => {
    ...
    return Object.assign(Object.assign({}, state), { isActive: true, 
        ...
        // @ts-ignore
        startY: action.mouseY, startPercentage: state.percentage, startValue: state.value });
};
const reduceOnMove = (state, action, callbacks) => {
    ...
    */
    ...
    let percentage = snapPercentage((state.startPercentage + (deltaX + deltaY) / (steps)), steps);
    console.debug("pr2", percentage);
    //const position2 = snapPosition(position, state, steps);
    const mouseAngle = (state.angleOffset + state.angleRange * percentage) % 360;
    let value = getValueFromPercentage(Object.assign(Object.assign({}, state), { percentage }));
    if (value > state.max) {
        percentage = 1;
        value = state.max;
    }
...
const reduceOnSet = (state, action, callbacks) => {
    ...
    return Object.assign(Object.assign({}, state), { value: newValue, percentage: percentage, callBackValue: null });
};
