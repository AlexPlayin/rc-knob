...
const reduceOnStart = (state, action, callbacks) => {
    ...
    return Object.assign(Object.assign({}, state), { isActive: true, 
        ...
        // @ts-ignore
        startY: action.mouseY, startPercentage: state.percentage, startValue: state.value, value, callBackValue: value });
};
const reduceOnMove = (state, action, callbacks) => {
    ...
    */
    ...
    const percentage = snapPercentage((state.startPercentage + (deltaX + deltaY) / (state.max - state.min)), steps);
    //console.log("pr2", percentage)
    //const position2 = snapPosition(position, state, steps);
    const mouseAngle = (state.angleOffset + state.angleRange * percentage) % 360;
    const value = getValueFromPercentage(Object.assign(Object.assign({}, state), { percentage }));
    callbacks.onInteractiveChange(value);
    if (state.tracking) ;
    //console.log(percentage)
    //console.log("EYY", mouseAngle, value, percentage)
    return Object.assign(Object.assign({}, state), { mouseAngle: mouseAngle < 0 ? mouseAngle + 360 : mouseAngle, percentage,
        //...position2,
...
const reduceOnStop = (state, action, callbacks) => {
    ...
    callbacks.onEnd();
    return Object.assign(Object.assign({}, state), { isActive: false, value: state.value, percentage: state.percentage, startPercentage: undefined, startValue: undefined, startX: undefined, startY: undefined });
};
const reduceOnCancel = (state, action, callbacks) => {
    ...
    return Object.assign(Object.assign({}, state), { isActive: false, value,
        percentage, callBackValue: value, startPercentage: undefined, startValue: undefined });
};
const reduceOnSteps = (state, action, callbacks) => {
    ...
    return Object.assign(Object.assign({}, state), { value, callBackValue: value, percentage: getPercentageFromValue(Object.assign(Object.assign({}, state), { value })) });
};
const reduceOnSet = (state, action, callbacks) => {
    if (state.value === null) {
        return state;
    }
    if (action.steps === null || action.steps === undefined)
        return state;
    const newValue = action.steps;
    const percentage = getPercentageFromValue(Object.assign(Object.assign({}, state), { value: newValue }));
    console.log("KNOB: In Dispatch on Set", newValue, percentage);
    return Object.assign(Object.assign({}, state), { value: newValue, percentage: percentage });
};
const reducer = (callbacks) => (state, action) => {
    switch (action.type) {
        ...
        case 'STEPS':
            return reduceOnSteps(state, action);
        case 'SET':
            return reduceOnSet(state, action);
        default:
...
var useUpdate = ({ min, max, multiRotation, initialValue, angleOffset = 0, angleRange = 360, size, steps, onChange, onInteractiveChange, interactiveHook, onStart, onEnd, readOnly, tracking, useMouseWheel, }) => {
    ...
    const [{ percentage, value, callBackValue }, dispatch] = React.useReducer(reducer(callbacks), {
        isActive: false,
        min,
        max,
        multiRotation,
